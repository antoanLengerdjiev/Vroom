...
{
    [TestFixture]
    public class GetFilteredUsersInRoleAsync
    {
        ...
        public void Setup()
        {
            ...
            this.mockedMapper.Setup(x => x.Map<List<ApplicationUser>, List<ApplicationUserServiceModel>>(It.IsAny<List<ApplicationUser>>())).Returns(this.collectionOfAppUserServiceModel);
        ...
        [TestCase("Admin")]
        [TestCase("Executive")]
        public async Task ShouldCallGetUsersInRoleAsyncMethodFromUserManager_WhenParamsAreValid(string role)
        {
            // Arrange

            // Act
            await this.userService.GetFilteredUsersInRoleAsync(role,1, 2, "", "");
        ...
        [TestCase("Admin")]
        [TestCase("Executive")]
        public async Task ShouldCallMapMethodFromMapper_WhenParamsAreValid(string role)
        {
            // Arrange
            ...
            // Assert
            this.mockedMapper.Verify(x => x.Map<List<ApplicationUser>, List<ApplicationUserServiceModel>>(It.IsAny<List<ApplicationUser>>()));
        }

        [TestCase("Admin",1, 2, "enix", 4)]
        [TestCase("Admin",1, 2, "Grin", 1)]
        [TestCase("Admin",1, 2, "len", 2)]
        [TestCase("Executive", 1, 2, "enix", 4)]
        [TestCase("Executive", 1, 2, "Grin", 1)]
        [TestCase("Executive",1, 2, "len", 2)]
        public async Task ShouldReturnCorrectTotalSize_WhenParamsAreValid(string role, int pageNumber, int pageSize, string searchPattern, int expectedResult)
        {
            // Arrange

            // Act
            var result = await this.userService.GetFilteredUsersInRoleAsync(role, pageNumber, pageSize, "", searchPattern);
        ...
        [TestCase("Admin")]
        [TestCase("Executive")]
        public async Task ShouldReturnTypeOfPageUserServiceModel_WhenParamsAreValid(string role)
        {
            // Arrange

            // Act
            var result = await this.userService.GetFilteredUsersInRoleAsync(role, 1, 2, "", "");

            // Assert
            Assert.IsInstanceOf<PagedUserServiceModel>(result);
        }


        [TestCase("Admin")]
        [TestCase("Executive")]
        public async Task ShouldReturnCorrectResult_WhenParamsAreValid(string role)
        {
            // Arrange
            ...
            // Assert
            Assert.AreEqual(this.collectionOfAppUserServiceModel, result.Users);
        }
    }
}
